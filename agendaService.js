...
const getAgenda = async (data) => {
  ...
  const pendente = (item) =>
    item.dt_conclusao === null && moment(item.dt_previsao).isSameOrBefore(data);
...
const getTarefas = async () => {
  ...
  return res.data.filter(
    (item) => item.dt_previsao === null && item.dt_conclusao === null
  );
};

const getConcluidas = async () => {
  const res = await axios.get(
    `${baseUrl}?_sort=dt_previsao,descricao&_order=asc`
  );
...
const concluirTarefa = async (id) => {
  ...
  const res = await axios.put(`${baseUrl}/${id}`, {
    ...tarefa,
    dt_conclusao: moment().format("YYYY-MM-DD"),
  });
  return res.data;
};
...
const atualizarDataTarefa = async (idTarefa, data) => {
  ...
  const res = await axios.put(`${baseUrl}/${idTarefa}`, {
    ...tarefa,
    dt_previsao: data.format("YYYY-MM-DD"),
  });
  return res.data;
};
